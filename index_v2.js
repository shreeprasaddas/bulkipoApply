...
async function applyIPO() {
    ...
    const ipoList = await page.evaluate(() => {
        ...
        companyItems.forEach((item, index) => {
            ...
            const fullText = companyNameDiv?.textContent || '';
    ...
    console.log('IPO List extracted:', ipoList);
    ...
    for (const ipo of filteredIPOs) {
        ...
        try {
            ...
            try {
                const bankSelect = await page.$('select[name="bankCode"], select[name="bank_code"], select[name="bank"]');
                if (bankSelect) {
                    const bankValue = await page.evaluate(() => {
                        const select = document.querySelector('select[name="bankCode"], select[name="bank_code"], select[name="bank"]');
                        return select?.options[1]?.value || select?.options[0]?.value;
                    });
                    await page.select('select[name="bankCode"], select[name="bank_code"], select[name="bank"]', bankValue);
                    console.log(`✓ Bank selected: ${bankValue}`);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                } else {
            ...
            try {
                const accountSelect = await page.$('select[name="accountCode"], select[name="account_code"], select[name="account"]');
                if (accountSelect) {
                    const accountValue = await page.evaluate(() => {
                        const select = document.querySelector('select[name="accountCode"], select[name="account_code"], select[name="account"]');
                        return select?.options[1]?.value || select?.options[0]?.value;
                    });
                    await page.select('select[name="accountCode"], select[name="account_code"], select[name="account"]', accountValue);
                    console.log(`✓ Account selected: ${accountValue}`);
                    await new Promise(resolve => setTimeout(resolve, 1000));
                } else {
                    console.log('Account select not found');
                }
            } catch (e) {
            ...
            await page.screenshot({ path: `ipo-form-before-proceed-${ipo.companyName.replace(/\s+/g, '-')}.png` });
            ...
            try {
                ...
                await page.screenshot({ path: `ipo-error-${ipo.companyName.replace(/\s+/g, '-')}.png` });
            }
        } catch (error) {
